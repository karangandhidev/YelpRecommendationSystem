...
def update_userid(value):
    ...
    html.Div([
        ...
        html.Div(children=[
            ...
            html.H4("User #{0}'s Reviews".format(value))
            ]+htmls, className="six columns",style={'float':'left','width':'48%','border':"2px black solid"}),

        html.Div(children=[
            html.H4('Recommendations for User #{0}'.format(value))]+preds, className="six columns",style={'float':'right','width':'48%','border':"2px black solid"}),
            ], className="row")
    ])
